...
public class TCPIPUtils {
    ...
    public TCPIPUtils(Socket socket) {
    ...
    public TCPIPUtils(ServerSocket serverSocket, Socket socket) {
        this(socket);
        this.serverSocket = serverSocket;
    }

    public static TCPIPUtils createClientSocket() {
        try {
            Socket socket = new Socket("localhost", 2020);
            System.out.println("Cliente> Establecida conexion");
            return new TCPIPUtils(socket);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static TCPIPUtils createServerSocket() {
        try {
            ...
            System.out.println("Servidor> Recibida conexion de " + socket.getInetAddress().getHostAddress() + ":"
                    + socket.getPort());
            return new TCPIPUtils(serverSocket, socket);
